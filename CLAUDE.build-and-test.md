...
- **Pre-build**: the root `globalSetup` (`vitest.setup.ts`) runs `turbo run build:dev --output-logs=errors-only` via `AgentPlugin.runScript` before **every** vitest run — CLI and the MCP `run_tests` tool alike — so tests always see fresh `dist/dev` artifacts. Turbo's cache makes it a fast no-op when nothing changed. Its path is resolved against `vitest.config.ts`, not the cwd (effected#455 — it was cwd-relative and died `ERR_LOAD_URL` on a `packages/<pkg>/vitest.setup.ts` that never existed). **Prefer `vitest run --project @effected/<pkg>`, which now works from any directory.** A bare `vitest run` from inside a package is correct but runs the whole repo; only a **positional** filter is cwd-sensitive, and not as a path — it is a substring matched against each test file's path as rendered from the cwd, so `packages/<pkg>` matches from the root and matches nothing from inside that package, while `__test__` matches everywhere. A miss collects nothing: `Tests: 0/0 passed` while exiting 1.
