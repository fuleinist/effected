...
- `cli/execute.ts` — the shared body: first, before the config loads, `--force` with an explicit `--drift` other than `allow` fails `ConflictingFlagsError` marked exit `64` (a usage error, never a silent resolution to `allow`); then load, merge flags over the config's onDrift and per-schema drift, run, emit, step summary, then `GateError`/`DriftError`, and for `check` a `StaleError` when any document would be written and an `OrphanedCatalogError` when a catalog file exists that no schema declares (all marked exit `1` — `check` is the CI gate, so a stale tree fails it). `DriftError` carries `drifted: [{ $id, change, version?, nextVersion? }]` — one entry per schema whose verdict is `drift`, `count` derived — and its message lists one line per schema.
- `ConfigLoader` — discovery and `jiti` loading of the config, `defineConfig` validation (exit `2`), plus the shape re-checks a forged brand could skip (a malformed schema target; a `frozen` entry missing `version`/`path`/`url`; a duplicate output path after resolving to an absolute path), each guarded ahead of the dereference it protects. Relative `outputDir`, `catalogPath`, each schema's current target `path`, and every frozen predecessor's `path` resolve against the config file's directory.
- `Runner` — the shared build/check walk over `SchemaPipeline`, drift table applied. Before anything is generated, every schema's `frozen` versions are checked for existence — a missing one fails typed `FrozenVersionMissingError` (exit `1`) before anything is written. A catalog entry is compared with ONE `readFileString` (`NotFound` → absent, unparseable → different) and the library's `CanonicalJson.equals`; no `exists` pre-check and no local `deepEqual`. Every schema's declared catalog entry lands in ONE file at `config.catalogPath`, never one file per schema. When NO schema declares one, `check` stats `catalogPath` and reports a file left there as `orphaned` (`OrphanedCatalogError`, exit `1`); `build` never reports and never deletes it.
