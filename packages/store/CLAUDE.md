...
- **`Cache.degrading(layer)` is the opt-in degrade-to-miss posture**, and the two halves of its cause handling are both load-bearing. It catches **defects** because `SqliteClient.layer` reports the common construction failure (a `filename` whose parent directory does not exist) as a defect rather than a typed failure — a failure-only catch misses exactly the case it exists for. It deliberately does **not** catch interruption: that is the caller shutting down, and swallowing it hands a working cache back to a fiber that was meant to stop. A hand-written `Layer.catchCause` gets the first half right and the second wrong, which is the whole reason this lives in the kit. Opt-in, never the default — a consumer wanting a fatal cache, or the narrower per-operation `orElseSucceed` on `get` with the layer left fatal, keeps that by not calling it, and the plain constructors' typed error channels are unchanged. The degraded shape is also the one exemption from the transactional `onRemoved` invariant above: nothing ever entered a degraded cache, so it runs no removal callbacks.
- **`CacheShape.degraded` tells a cold cache from a broken one.** Every `get` misses either way, so without it degradation is invisible. It is a plain field rather than a `CacheEventPayload` member because degradation is decided at construction, before any subscriber exists — an event published then reaches nobody, since the `events` hub does not replay.
