...
- `unverified` says why the report is not a complete answer: `"peerRulesNotApplied"` when the suppression policy could not be applied, `"unresolvedEdge"` when an instance records an edge the model could not name or an importer dependency resolved through `link:` — a linked parent's manifest peers are never in the lockfile, though `pnpm peers check` reads them from disk. Both mean fail closed.
