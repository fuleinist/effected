...
- `@./okf/modules/yaml.md` — load when changing the public API, the engine seams or the hardening guards.
- `@./okf/interfaces/yaml-comment-model.md` — load when changing where a comment is captured, which node owns it, or how the stringifier puts it back.
- `@./okf/interfaces/yaml-stringify-options.md` — load when changing an emitter option: sequence indentation, explicit keys, folding, re-quoting or 1.1 compatibility.
- `@./okf/interfaces/yaml-lint.md` — load when changing the token stream, the rule model, a built-in rule or autofix.
