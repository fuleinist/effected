...
- **`CanonicalJson`** - the deterministic serializer: insertion-order keys (assembly owns ordering, nothing is sorted), tab indent by default (`CanonicalJsonOptions.indent`), LF, single trailing newline. Fails typed where `JSON.stringify` silently drops or rewrites: `NonJsonValueError` (with a JSON pointer) and `JsonDepthExceededError` (also catches cycles). `CanonicalJsonError` is the union. **`CanonicalJson.equals(left, right)`** is the matching content predicate: parsed-content equality with the serializer's own semantics (key order never decides, element order always does, primitives exact so `NaN` never equals, serializer-refused values unequal unless the identical reference) - consumers comparing documents or `JSON.parse` results by content never hand-roll a deep-equal.
