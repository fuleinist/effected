...
- **`check`** is the identical walk with no writes: it reports exactly what `build` would do under the same flags (`would write`, `unchanged`, `DRIFT`, `held`, `GATE FAILED`) and exits under the same conditions — and, as the CI gate, it also exits `1` whenever a build would write anything (`StaleError`: ``N document(s) are stale; run `schemastore build` and commit the result.``), evaluated after the gate and drift verdicts. A `catalog.json` no schema declares is reported `orphaned` and fails `check` the one way `build` cannot clear: delete the file by hand, or restore a `catalog` block — `build` never deletes it. It replaces a hand-written drift test.
