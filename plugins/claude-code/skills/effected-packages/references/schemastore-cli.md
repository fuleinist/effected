...
- **`check`** is the identical walk with no writes: it reports exactly what `build` would do under the same flags (`would write`, `unchanged`, `DRIFT`, `held`, `GATE FAILED`) and exits under the same conditions — and, as the CI gate, it also exits `1` whenever a build would write anything (`StaleError`: ``N document(s) are stale; run `schemastore build` and commit the result.``), evaluated after the gate and drift verdicts, or when an output nothing claims sits on disk — an orphaned `catalog.json` no schema declares, or a `*.json` document in a directory the config writes into that no target, frozen version, or catalog path names (a rename moved its derived path); `build` reports both and deletes neither — delete by hand. It replaces a hand-written drift test.
