...
The predicate means "proven clean", and under pnpm it is only reachable for a workspace with no `link:`-resolved importer dependencies: every `workspace:` dependency is recorded `link:`, and a linked parent's manifest peers are never in the lockfile, so a pnpm monorepo with internal dependencies reports `unverified: ["unresolvedEdge"]`. A gate that must pass on such a workspace pairs the report with `pnpm peers check` rather than dropping the `unverified` clause.
...
- **`PeerCheck.run` fails closed, and an empty `unsatisfied` is NOT the same as clean.** The real "proven clean" predicate is `supported && !report.unresolvedImporters.length && !report.unverified.length && !report.required.length` — check all four, not just `unsatisfied`. `unverified` is `"peerRulesNotApplied" | "unresolvedEdge"`; both mean fail closed. `"unresolvedEdge"` also fires for every `link:`-resolved importer dependency — under pnpm that is every `workspace:` dependency, because a linked parent's manifest peers are never in the lockfile — so a pnpm workspace with internal dependencies is unverified structurally, until issue 800's option 1 joins linked manifests from disk.
