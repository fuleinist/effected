...
Read `CLAUDE.md` at the repo root first. It is the map: it names every package, its tier, and its non-negotiables, and points at per-package `CLAUDE.md` files and the `okf/` knowledge bundle. Read the target package's own `CLAUDE.md` before touching its source. Those files override anything here.
...
The body is ordinary markdown — headings, bullets, tables and code fences are all fine. Two rules apply: never cite `.claude/plans/` or any other local working file, since this repository is public and those paths are not readable by the people who will see the pull request (committed `okf/` concepts may be linked); and link issues with a bare `Closes #N` on its own line, outside every fence, because a reference inside a fenced block is inert.
