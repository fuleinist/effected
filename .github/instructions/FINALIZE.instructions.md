...
* **plan-leakage** — do not cite `.claude/plans/` or write "as decided in the plan".
  Plans are local working files, not something a reader can open. Restate the reasoning in
  the summary instead of pointing at a path. Concepts under `okf/` are committed and may be
  linked when they carry the rationale.
